...
contract Staking_mock is ParachainStaking {
    ...
    function scheduleDelegatorBondLess(address candidate, uint256 less)
        external {
            require(scheduledBondLess[msg.sender][candidate] == 0, "ALREADY_SCHEDULED");
            require(delegations[msg.sender][candidate] - less >= MINIMUM_DELEGATION, "BELOW_MIN");
    ...
    function executeDelegationRequest(address delegator, address candidate)
        external {
            require(scheduledBondLess[delegator][candidate] > 0 || scheduledRevoke[delegator][candidate], "NO_PENDING");
            if (scheduledBondLess[delegator][candidate] > 0) {
                delegations[delegator][candidate] -= scheduledBondLess[delegator][candidate];
    ...
    function cancelDelegationRequest(address candidate) external {
            require(scheduledBondLess[msg.sender][candidate] > 0 || scheduledRevoke[msg.sender][candidate], "NO_PENDING");
